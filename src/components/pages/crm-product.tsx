...
import { RevenueOpsFlow } from "@/components/motion/revenue-ops-flow";
import { CustomerTimeline } from "@/components/motion/customer-timeline";
...
function PipelineCard({ name, value, tag, accent = false }: { name: string; value: string; tag: string; accent?: boolean }) {
  return (
    <div className={`rounded-xl border p-2.5 shadow-sm ${accent ? "border-primary/30 bg-primary-soft" : "border-border bg-card"}`}>
    ...
    <div className="relative mx-auto w-full max-w-lg">
      ...
      <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-2xl shadow-black/[0.1]">
        <div className="flex items-center gap-2 border-b border-border bg-card-2 px-4 py-3">
          ...
          <span className="ml-3 flex-1 truncate rounded-md bg-card px-3 py-1 text-[11px] font-medium text-muted-foreground">OZZO · Sales pipeline</span>
        </div>
        <div className="grid grid-cols-4 gap-2 p-4">
          {[{ h: "New", n: 8 }, { h: "Contacted", n: 5 }, { h: "Quoted", n: 3 }, { h: "Won", n: 2 }].map((col) => (
            <div key={col.h} className="min-w-0">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{col.h}</span>
                <span className="rounded-full bg-muted px-1.5 text-[9px] font-bold text-muted-foreground">{col.n}</span>
              </div>
              <div className="space-y-2">
                {col.h === "New" && (<><PipelineCard name="Vertex Electricals" value="₹2.4L" tag="Website" accent /><PipelineCard name="Ganesh Stores" value="₹68k" tag="WhatsApp" /></>)}
                {col.h === "Contacted" && <PipelineCard name="Meera Traders" value="₹1.1L" tag="Referral" />}
                {col.h === "Quoted" && <PipelineCard name="Anand Hardware" value="₹3.2L" tag="Field" accent />}
                {col.h === "Won" && <PipelineCard name="Skyline Infra" value="₹5.0L" tag="Repeat" />}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="absolute -bottom-6 -left-4 w-60 rounded-2xl border border-border bg-card p-3 shadow-xl shadow-black/[0.1] animate-float">
        <div className="flex items-center gap-2">
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-success/15 text-success"><MessageCircle className="h-4 w-4" /></span>
        ...
        <div className="mt-2 flex items-start gap-1.5 rounded-lg border border-primary/20 bg-primary-soft px-2 py-1.5">
          ...
          <p className="text-[10px] leading-snug text-foreground"><span className="font-semibold text-primary">AI</span> replied to “lead time?”</p>
        </div>
      </div>
      <div className="absolute -right-3 top-8 rounded-2xl border border-border bg-card px-3 py-2 shadow-xl shadow-black/[0.1] animate-float" style={{ animationDelay: "1.4s" }}>
    ...
    <div className="relative mx-auto w-full max-w-xl">
      ...
      <div className="overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-2xl shadow-black/[0.08] md:p-8">
        <div className="grid grid-cols-3 gap-3">
          {[{ label: "New leads", value: "128" }, { label: "Deals won", value: "37" }, { label: "Quotes sent", value: "54" }].map((s) => (
        ...
        <div className="mt-5 grid grid-cols-[1.4fr_1fr] gap-4">
          <div className="rounded-2xl border border-border bg-card-2 p-4">
            ...
            <div className="mt-4 flex h-28 items-end gap-2">
              {bars.map((h, i) => (<div key={i} className="flex-1 rounded-t bg-gradient-to-t from-primary/40 to-primary" style={{ height: `${h}%` }} />))}
            </div>
          </div>
          <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-card-2 p-4">
            ...
            <div className="relative h-24 w-24 rounded-full" style={{ background: "conic-gradient(var(--primary) 0 45%, oklch(0.62 0.22 305) 45% 72%, var(--border) 72% 100%)" }}>
...
function Cell({ v, highlight = false }: { v: boolean | string; highlight?: boolean }) {
  ...
  if (v === false)
    return (
      <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-muted text-muted-foreground/50"><X className="h-3.5 w-3.5" /></span>
    );
  return <span className={`text-xs font-semibold ${highlight ? "text-primary" : "text-muted-foreground"}`}>{v}</span>;
}
...
export function CrmProductPage() {
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-ink pt-32 pb-16 text-white md:pt-40 md:pb-20">
        ...
        <Container className="relative z-10">
          <div className="grid items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <Reveal delay={80}>
                <h1 className="ozzo-display mt-4 max-w-2xl text-[2.4rem] leading-[1.05] text-white sm:text-5xl md:text-[3.5rem]">
                  ...
                  <span className="ozzo-gradient-bright">office to field</span>, on one record.
                </h1>
              </Reveal>
              <Reveal delay={160}>
            ...
            <Reveal delay={200} variant="scale"><CrmDashboardVisual /></Reveal>
          </div>
        </Container>
      </section>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Reveal delay={crmProblems.length * 60}>
              <div className="flex h-full flex-col justify-center rounded-3xl border border-primary/20 bg-primary-soft p-7">
                ...
                <p className="mt-3 text-lg font-bold leading-snug text-foreground">One system where every lead has an owner, a next step, and a full history.</p>
                <Link href="#engine" className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary">See the flow <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </Reveal>
          </div>
        </Container>
      ...
      {/* ─────────── SECTION 4 · WhatsApp + AI (animated thread) ─────────── */}
      <section className="py-24 md:py-28">
        <Container>
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <Reveal variant="left">
              <Eyebrow>WhatsApp CRM</Eyebrow>
              <h2 className="ozzo-display mt-4 text-3xl text-foreground md:text-[2.5rem] md:leading-[1.1]">Every conversation in one inbox — with AI on the routine questions.</h2>
              <p className="mt-5 text-lg leading-relaxed text-muted-foreground">
                Chats on your official WhatsApp number land in one shared team inbox,
                attached to the right lead — not stuck on a personal phone. Templates keep
                replies fast, and an AI assistant answers routine questions from your own
                knowledge base.
              </p>
              <ul className="mt-7 space-y-3.5">
                {[
                  "Shared team inbox with reactions & unread indicators",
                  "Message templates for fast, consistent replies",
                  "AI assistant answers from documents you upload — only from what you give it",
                  "Every message on the customer's timeline",
                ].map((p) => (
                  <li key={p} className="flex items-start gap-3">
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary-soft text-primary"><Check className="h-3 w-3" strokeWidth={3} /></span>
                    <span className="text-[15px] leading-relaxed text-foreground">{p}</span>
                  </li>
                ))}
              </ul>
            </Reveal>
            <Reveal variant="right" delay={100}><WhatsAppThread /></Reveal>
          </div>
        </Container>
      </section>

      {/* ─────────── SECTION 5 · Customer timeline (animated) ─────────── */}
      <section className="border-y border-border bg-card-2 py-24 md:py-28">
        <Container>
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <Reveal variant="right" delay={100} className="lg:order-2"><CustomerTimeline /></Reveal>
            <Reveal variant="left" className="lg:order-1">
              ...
              <ul className="mt-7 space-y-3.5">
                {[
                  "Leads, deals, tasks and activities on one feed",
                  "Geo-tagged field visits on the same record",
                  "Branded quotations with a full version trail",
                  "The database belongs to the business — not the rep who leaves",
                ].map((p) => (
                  <li key={p} className="flex items-start gap-3">
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary-soft text-primary"><Check className="h-3 w-3" strokeWidth={3} /></span>
                    <span className="text-[15px] leading-relaxed text-foreground">{p}</span>
        ...
        <Container>
          <div className="grid items-center gap-14 lg:grid-cols-2">
            <Reveal variant="left">
              ...
              <h2 className="ozzo-display mt-4 text-3xl text-foreground md:text-[2.6rem] md:leading-[1.1]">Manage by facts, not the loudest update.</h2>
              <p className="mt-5 text-lg leading-relaxed text-muted-foreground">
                A report engine where you choose the dimensions, measures and filters, save
                a default view, and export. Lead, Deal and Quotation reports are ready out
                of the box — part of a suite of 11 built-in reports.
              </p>
              <ul className="mt-7 space-y-3.5">
                {[
            ...
            <Reveal variant="right" delay={100}><ReportsVisual /></Reveal>
          </div>
        </Container>
      </section>
      ...
      <section className="relative overflow-hidden bg-ink py-24 text-white md:py-28">
        ...
        <Container className="relative z-10">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="ozzo-display mt-4 text-4xl text-white md:text-5xl">Built to work with <span className="ozzo-gradient-text">AI assistants</span>.</h2>
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-2">
            <Reveal variant="left">
              <div className="h-full rounded-3xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur">
                <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-success/15 px-3 py-1 text-xs font-bold text-success"><span className="h-1.5 w-1.5 rounded-full bg-success" /> Live today</div>
                <h3 className="text-xl font-bold text-white">The AI knowledge-base assistant</h3>
                <p className="mt-3 text-[15px] leading-relaxed text-white/60">Upload your own knowledge documents and the assistant answers routine customer questions on WhatsApp from them — instantly, and only from what you gave it. Your team is freed for the conversations that need a person.</p>
              </div>
            </Reveal>
            <Reveal variant="right" delay={100}>
              <div className="h-full rounded-3xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur">
                <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/20 px-3 py-1 text-xs font-bold text-primary"><Sparkles className="h-3.5 w-3.5" /> Built for what's next</div>
                <h3 className="text-xl font-bold text-white">Your data, ready for AI</h3>
                <p className="mt-3 text-[15px] leading-relaxed text-white/60">Structured leads, deals, quotes and a single customer timeline — not scattered chats and spreadsheets. As AI assistants become part of how teams sell, clean, connected data is the foundation they run on, and it&apos;s yours to own and export.</p>
              </div>
            </Reveal>
          </div>
          <p className="mx-auto mt-8 max-w-2xl text-center text-sm text-white/40">We only describe what OZZO does today. We won&apos;t promise AI features that aren&apos;t yet shipped.</p>
        </Container>
      </section>
      ...
      <section className="py-24 md:py-28">
        <Container>
          <SectionHeading eyebrow="Why businesses choose OZZO" title="Spreadsheet, generic CRM, or OZZO" description="What actually changes when the pipeline lives somewhere built for how field-sales teams work." />
          <Reveal>
                ...
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="p-5 text-center text-sm font-bold text-primary">
                      <span className="inline-flex items-center gap-1.5"><span className="flex h-5 w-5 items-center justify-center rounded-md bg-primary text-[11px] text-primary-foreground">O</span>OZZO CRM</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
              ...
              <div className="flex h-full flex-col gap-4">
                ...
                <div className="mt-auto rounded-3xl border border-dashed border-border p-6">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {includedInEveryPlan.map((b) => (<span key={b} className="rounded-full border border-border bg-card-2 px-2.5 py-1 text-xs font-medium text-muted-foreground">{b}</span>))}
                  </div>
                </div>
              </div>
            </Reveal>
